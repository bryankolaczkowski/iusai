...
class GenEnc(tf.keras.layers.Layer):
  ...
  """
  def __init__(self, dim, mmult=2., *args, **kwargs):
    super(GenEnc, self).__init__(*args, **kwargs)
    self.dimn = dim
    self.mmlt = mmult
    self.mult = tf.constant(self.mmlt*2, dtype=tf.float32)
    self.subt = tf.constant(self.mmlt, dtype=tf.float32)
    return
  ...
  def call(self, inputs):
    ...
    inputs should be 0|1 labels
    """
    ms = tf.cast(inputs, dtype=tf.float32) * self.mult - self.subt
    return tf.random.normal(shape=(tf.shape(inputs)[0],self.dimn), mean=ms)
...
## generator build ##
def generator_build(outdim, repdim=8, nblcks=4, nheads=4, rndsprd=2.):
  ...
  out = GenEnc(outdim, mmult=rndsprd, name='encod')(inputs)
...
if __name__ == '__main__':
  ...
  group.add_argument('--rndspread', dest='rndspread', type=float,
                     help='conditional generator spread', metavar='N')
